        ...
        <div className="xl:flex">
          <div className="flex flex-wrap justify-center mt-6 gap-4 ">
            {person.slice(0, 2).map((item) => (
              <div
                key={item.id}
                className="w-[150px] md:w-[200px] lg:w-[250px] xl:w-[404px]  text-start relative"
              >
                <img
                  src={`https://image.tmdb.org/t/p/w500${item.profile_path}`}
                  alt={item.name}
                  className="w-full xl:rounded-[13px] xl:h-[450px] xl:w-[500px] rounded-[13px] mb-2"
                />
          ...
          <div className="bg-[#1B2133] w-full rounded-[10px] p-5 mt-2 xl:w-[30%] xl:mt-[25px]">
            {person.slice(2, 7).map(
              (item, index, random) => (
                (random = Math.floor(Math.random() * (70 - 20 + 1)) + 20),
                (
